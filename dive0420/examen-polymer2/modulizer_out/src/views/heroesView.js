 ...
 * @polymer
 */
class HeroesView extends ReduxMixin(PolymerElement) {
    static get template() {
        return html`
    <style>
      *  {
    ...
    <div class="options-container">
        ...
        <template is="dom-if" if="[[characters.length]]">
    ...
    <ul id="characters" class="grid-container">
      <template is="dom-if" if="[[characters.length]]">
        <template id="list" is="dom-repeat" items="[[characters]]" filter="{{_filter(filterVal)}}" sort="{{_sort(sortVal)}}" observe="name">
          <li>
...
`;
    ...
    static get is() { return 'heroes-view'; }
    ...
    handleCharacterResponse(event){
        const characters = event.detail.response.data.results;
...
window.customElements.define(HeroesView.is, HeroesView);
